...
14. **サイズが計算できないレコード（`length`/`width`/`height` がNULLまたは全て0）がある場合、CSVからスキップされ警告メールが送信される**
...
20. **設定ファイルが存在しない場合、エラーログが出力され終了する**
...
22. **20万件のCSVが正常に生成され `jp_download` が更新される**
...
23. **20万件処理後に再実行した場合、対象なしで正常終了する**
    - 前提: 22番実行済み（全対象レコードの `jp_download = 1` 更新済み）
    - 操作: バッチ再実行
...
24. **バッチ実行中に同じバッチを起動しようとした場合、二重起動が防止されメールが送信される**
...
25. **バッチが正常終了した後、ロックファイルが削除される**
    - 前提: 送信対象レコードがDBに存在する
    - 操作: バッチ実行・完了
    - 期待: `work/put_jp_edi.lock` が存在しない

26. **バッチが異常終了した後、ロックファイルが残り手動削除後に再実行可能になる**
