...
12. **No.11（ご依頼主名）が `goods_hawb.shipper_name` から取得される**
    - 前提: `shipper_name = '山田 太郎'` のレコードをDBに用意する、`sftp_put_enabled = false`
    - 操作: バッチ実行後、CSVのNo.11を確認
    - 期待: No.11が `山田 太郎` になっている

13. **No.12〜14（ご依頼主電話番号・郵便番号・住所）が空欄になっている**
    - 前提: 送信対象レコードがDBに存在する、`sftp_put_enabled = false`
    - 操作: バッチ実行後、CSVのNo.12〜14を確認
    - 期待: No.12・13・14がすべてブランクになっている

14. **`jp_download` 更新後に再実行した場合、CSVに対象レコードが含まれない**
    - 前提: 4番実行済み（`jp_download = 1` に更新済み）
    - 操作: バッチ再実行
    - 期待: 対象レコードが0件となり `No records to send.` で終了する

15. **フィールドに禁則文字が含まれる場合、除去されてCSVに出力される**
    - 前提: `consignee_name` にタブ文字や改行が含まれるレコードをDBに用意する
    - 操作: バッチ実行（`sftp_put_enabled = false`）
    - 期待: CSVの該当フィールドから禁則文字が除去されている

16. **サイズが計算できないレコード（`length`/`width`/`height` がNULLまたは全て0）がある場合、CSVからスキップされ警告メールが送信される**
    - 前提: `length = NULL`, `width = NULL`, `height = NULL` のレコードをDBに用意する
    - 操作: バッチ実行
    - 期待: 該当レコードがCSVに含まれない / ログに `Skipped (size unresolvable)` が出力される / 警告メールが送信される

17. **サイズコードの境界値が正しく計算される**
...
18. **サイズが170cm超のレコードがある場合、No.27が `170` になり警告メールは送信されない**
    - 前提: 3辺合計が170cm超（例: 60+60+60=180）のレコードをDBに用意する
    - 操作: バッチ実行（`sftp_put_enabled = false`）
    - 期待: No.27が `170` で出力される / 警告メールは送信されない

19. **`tracking_no=NULL` かつ `shipping_club='ゆうパック'` のレコードが対象になる**
    - 前提: `tracking_no=NULL`・`shipping_club='ゆうパック'` のレコードをDBに用意する、`sftp_put_enabled = false`
    - 操作: バッチ実行後、CSVを確認
    - 期待: 該当レコードがCSVに含まれる / No.7（お問い合わせ番号）がブランクになっている

---

## エラー系

20. **SFTPエラー時にCSVが `put_error/` へ移動し終了する**
    - 前提: SFTPの接続情報を意図的に誤った設定にする
    - 操作: バッチ実行
    - 期待: CSVが `put_error/` へ移動する / `put_outbox/` にCSVが残らない / エラーログが出力される / エラーメールが送信される / 終了コード1

21. **DB接続エラー時にエラーメールが送信され終了する**
    - 前提: DBの接続情報を意図的に誤った設定にする
    - 操作: バッチ実行
    - 期待: CSVが生成されない / エラーメールが送信される / 終了コード1

22. **S3バックアップ失敗時に `put_backup/` にファイルが残り、次回起動時にリトライされる**
    - 前提: S3の接続情報を意図的に誤った設定にする（SFTP送信は成功する設定）
    - 操作: バッチ実行
    - 期待: CSVが `put_backup/` に残る / 次回バッチ起動時にS3バックアップがリトライされる

23. **設定ファイルが存在しない場合、エラーログが出力され終了する**
    - 前提: `config/settings.ini` を削除または別名にリネームする
    - 操作: バッチ実行
    - 期待: `CRITICAL` ログが出力される / CSVが生成されない / ロックファイルが解放される / 終了コード1
    - 備考: `settings.ini` がない場合はメール設定も読めないためエラーメールは送信不可（`Failed to send error mail.` がログに出力される）

24. **`mail_enabled = false` の場合、エラー発生時もメールが送信されない**
...
25. **20万件のCSVが正常に生成され `jp_download` が更新される**
    - 前提: 送信対象レコード20万件がDBに存在する（`insert_test_200k.sql` 参照）、`sftp_put_enabled = false`、`s3_backup_enabled = false`
    - 操作: バッチ実行
    - 期待: CSVが生成される / `jp_download` が163,636件更新される（36,364件はサイズNGでスキップ）/ ログに `Generated CSV: ... (163636 rows, 36364 skipped)` が出力される / 終了コード0
    - 結果: 約21秒で完了（DB取得11秒 / CSV生成3秒 / jp_download更新7秒）✅

26. **20万件処理後に再実行した場合、対象なしで正常終了する**
    - 前提: 25番実行済み（全対象レコードの `jp_download = 1` 更新済み）
    - 操作: バッチ再実行
    - 期待: DBのレコード数が変化しない / ログに `No records to send. Exiting.` が出力される / 終了コード0 ✅

---

## 二重起動防止

27. **バッチ実行中に同じバッチを起動しようとした場合、二重起動が防止されメールが送信される**
...
28. **バッチが正常終了した後、ロックファイルが削除される**
    - 前提: 送信対象レコードがDBに存在する
    - 操作: バッチ実行・完了
    - 期待: `work/put_jp_edi.lock` が存在しない

29. **バッチが異常終了した後、ロックファイルが残り手動削除後に再実行可能になる**
